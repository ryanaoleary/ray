...
def _install_tpu_slice_fakes(
    ...
    *,
    ...
    selected_nodes = nodes or _create_mock_v6e_nodes(
...
def test_validation_matrix_tpu():
    ...
    # 7. TPU + placement_group_config -> rejects at config construction
    with pytest.raises(
        ValueError,
        match="placement_group_config is not supported for topology-backed TPU",
    ):
        vLLMEngineProcessorConfig(
            model_source="test-model",
            accelerator_type="TPU-V6E",
...
def test_builder_to_backend_tpu(mock_tpu_slice_environment):
    ...
    assert processor._close_handle is fake_handle
...
def test_direct_backend_validation_failures(monkeypatch):
    ...
    # 8. Backend backstop placement_group_config rejection
    with pytest.raises(ValueError, match="placement_group_config is not supported"):
        backend.build_batch_scheduling_plan(
            BatchSchedulingRequest(
                accelerator_type="TPU-V6E",
                accelerator_config=TPUConfig(topology="4x4"),
