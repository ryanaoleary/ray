...
class vLLMEngineProcessorConfig(_vLLMEngineProcessorConfig, ProcessorConfig):
    ...
    Args:
        ...
        placement_group_config: Optional placement group config for scheduling
            vLLM engine workers. For ordinary placement-group scheduling,
            ``bundle_per_worker`` is replicated by ``tp*pp`` (or use ``bundles``
            for the full list). For topology-backed TPU scheduling these fields
            are the homogeneous SlicePG worker template; the topology determines
            the resulting bundle count. Optional ``strategy``:
            ``PACK``/``STRICT_PACK``/``SPREAD``/``STRICT_SPREAD``.
        accelerator_config: Optional accelerator backend configuration,
            discriminated by ``kind``. TPU batch inference requires
            ``kind="tpu"`` and a ``topology``.
