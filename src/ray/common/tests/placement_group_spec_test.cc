...
TEST(PlacementGroupSpecTest, TestPlacementGroupSpecBuilderSchedulingOptions) {
  ...
  ASSERT_EQ(proto.placement_group_scheduling_options().options_size(), 2);

  const auto &p_option1 = proto.placement_group_scheduling_options().options(0);
  ASSERT_EQ(p_option1.label_selector().label_constraints_size(), 1);
  ASSERT_EQ(p_option1.label_selector().label_constraints(0).label_key(), "type");
  ASSERT_EQ(p_option1.label_selector().label_constraints(0).label_values(0), "A100");
  ASSERT_EQ(p_option1.bundles_size(), 1);
  ASSERT_EQ(p_option1.bundles(0).unit_resources().at("CPU"), 1.0);

  const auto &p_option2 = proto.placement_group_scheduling_options().options(1);
  ASSERT_EQ(p_option2.label_selector().label_constraints_size(), 1);
  ASSERT_EQ(p_option2.label_selector().label_constraints(0).label_key(), "type");
  ASSERT_EQ(p_option2.label_selector().label_constraints(0).label_values(0), "V100");
